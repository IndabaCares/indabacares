...
```
Root layout pre-loads two fonts via `useFonts()`: `DancingScript_700Bold` (from `@expo-google-fonts/dancing-script`) and `Ionicons.font` (from `@expo/vector-icons`). App renders with system font fallback until loaded. **Ionicons is also embedded natively via the `expo-font` config plugin in `app.json`** — this is required on iOS 26 New Architecture because the JS-layer `ExpoFontLoader` path silently fails (the lazy-Proxy patch prevents a crash but the font never actually loads), leaving every `<Ionicons>` component blank across all screens. The native plugin embeds `Ionicons.ttf` directly into the binary so the font is always available before any JS runs. Adding new icon families requires both spreading `IconFamily.font` into `useFonts()` AND adding the `.ttf` path to the `expo-font` plugin fonts array in `app.json`.
